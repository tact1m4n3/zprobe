...
pub const Vtable = struct {
    ...
    read: ?*const fn (ptr: *anyopaque, addr: u64, data: []u8) ReadError!void = null,
    write: ?*const fn (ptr: *anyopaque, addr: u64, data: []const u8) WriteError!void = null,
};
...
pub fn read(memory: Memory, addr: u64, data: []u8) ReadError!void {
    if (memory.vtable.read) |read_fn| {
        try read_fn(memory.ptr, addr, data);
    } else {
        try memory.read_u8(addr, data);
    }
}

pub fn write(memory: Memory, addr: u64, data: []const u8) WriteError!void {
    if (memory.vtable.write) |write_fn| {
        try write_fn(memory.ptr, addr, data);
    } else {
