...
fn create_libusb(
    ...
    lib.root_module.addCSourceFiles(.{ .files = src, .root = root });

    if (is_posix)
        lib.addCSourceFiles(.{ .files = posix_platform_src, .root = root });

    if (target.result.isDarwinLibC()) {
        lib.addCSourceFiles(.{ .files = darwin_src, .root = root });
        lib.linkFramework("CoreFoundation");
        lib.linkFramework("IOKit");
        lib.linkFramework("Security");
    } else if (target.result.os.tag == .linux) {
        lib.addCSourceFiles(.{ .files = linux_src, .root = root });
        if (system_libudev) {
            lib.addCSourceFiles(.{ .files = linux_udev_src, .root = root });
            lib.linkSystemLibrary("libudev");
        }
    } else if (target.result.os.tag == .windows) {
        lib.addCSourceFiles(.{ .files = windows_src, .root = root });
        lib.addCSourceFiles(.{ .files = windows_platform_src, .root = root });
    } else if (target.result.os.tag == .netbsd) {
        lib.addCSourceFiles(.{ .files = netbsd_src, .root = root });
    } else if (target.result.os.tag == .openbsd) {
        lib.addCSourceFiles(.{ .files = openbsd_src, .root = root });
    } else if (target.result.os.tag == .haiku) {
        lib.addCSourceFiles(.{ .files = haiku_src, .root = root });
    } else if (target.result.os.tag == .solaris) {
        lib.addCSourceFiles(.{ .files = sunos_src, .root = root });
    } else unreachable;

    lib.addIncludePath(root.path(b, "libusb"));
    lib.installHeader(root.path(b, "libusb/libusb.h"), "libusb.h");

    // config header
    if (target.result.isDarwinLibC()) {
        lib.addIncludePath(root.path(b, "Xcode"));
    } else if (target.result.abi == .msvc) {
        lib.addIncludePath(root.path(b, "msvc"));
    } else if (target.result.abi == .android) {
        lib.addIncludePath(root.path(b, "android"));
    } else {
        const config_h = b.addConfigHeader(.{ .style = .{
            ...
            .HAVE_IOKIT_USB_IOUSBHOSTFAMILYDEFINITIONS_H = define_from_bool(target.result.isDarwinLibC()),
